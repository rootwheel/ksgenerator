...
cp ks $PXEPATH
chown sftpuser. $PXEPATH/ks7

echo "KS file will be removed in 10 minutes according to at job"
at now +10 minute <<< "rm -f $PXEPATH/ks7" 2>/dev/null

printf '%s\n' "[$(date --rfc-3339=seconds)]: $*logged user $(last -1 | awk 'NR==1 {print $1,$3; exit}') creates a KS7 file for $dsuname $ipaddr $rootpw " >> /var/log/ksgen.log
